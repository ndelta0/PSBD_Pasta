...
create table pasta_db.class_types
(
...
create table pasta_db.schedule_entries
(
    id          int auto_increment
        primary key,
    userId      int                                                                                 not null,
    ...
    constraint schedule_entries_class_types_id_fk
        foreign key (classTypeId) references pasta_db.class_types (id)
            on delete set null,
...
create table pasta_db.semesters
(
    id        int auto_increment
        primary key,
    userId    int                  not null,
...
create table pasta_db.sessions
(
...
create table pasta_db.subjects
(
    id                int auto_increment
        primary key,
    userId            int                  not null,
    ...
    constraint subjects_semesters_id_fk
        foreign key (semesterId) references pasta_db.semesters (id)
            on delete set null,
...
create table pasta_db.attendance_record
(
    id              int auto_increment
        primary key,
    userId          int        not null,
    ...
    constraint attendance_record_schedule_entries_id_fk
        foreign key (scheduleEntryId) references pasta_db.schedule_entries (id)
            on delete set null,
    constraint attendance_record_subjects_id_fk
        foreign key (subjectId) references pasta_db.subjects (id)
            on delete cascade,
...
create table pasta_db.subject_class_types
(
    subjectId   int not null,
...
create table pasta_db.subject_teachers
(
    id        int auto_increment
        primary key,
    subjectId int          not null,
...
create definer = root@`%` view pasta_db.v_attendance_latest as
select `ar`.`userId`     AS `userId`,
       `ar`.`classDate`  AS `classDate`,
       `ar`.`wasPresent` AS `wasPresent`,
       `s`.`name`        AS `subjectName`,
       `ct`.`label`      AS `classType`
from (((`pasta_db`.`attendance_record` `ar` join `pasta_db`.`subjects` `s`
        on ((`ar`.`subjectId` = `s`.`id`))) left join `pasta_db`.`schedule_entries` `se`
       on ((`ar`.`scheduleEntryId` = `se`.`id`))) left join `pasta_db`.`class_types` `ct`
      on ((`se`.`classTypeId` = `ct`.`id`)))
order by `ar`.`classDate`;

create definer = root@`%` view pasta_db.v_attendance_summary as
select `ar`.`userId`                    AS `userId`,
       `s`.`name`                       AS `subjectName`,
       count(`ar`.`id`)                 AS `attendanceCount`,
       sum((`ar`.`wasPresent` = true))  AS `numPresent`,
       sum((`ar`.`wasPresent` = false)) AS `numAbsent`
from (`pasta_db`.`attendance_record` `ar` left join `pasta_db`.`subjects` `s` on ((`ar`.`subjectId` = `s`.`id`)))
group by `ar`.`subjectId`, `ar`.`userId`;

create definer = root@`%` view pasta_db.v_subject_attendance_summary as
select `ar`.`userId`                                                AS `userId`,
...
create definer = root@`%` view pasta_db.v_events_upcoming as
select `e`.`userId`      AS `userId`,
       ...
       `e`.`tone`        AS `tone`,
       `s`.`name`        AS `subjectName`
from (`pasta_db`.`events` `e` left join `pasta_db`.`subjects` `s` on ((`e`.`subjectId` = `s`.`id`)))
order by `e`.`eventDate`, `e`.`eventTime`;

create definer = root@`%` view pasta_db.v_grade_details as
select `g`.`id`        AS `gradeId`,
       ...
       `g`.`name`      AS `name`
from (`pasta_db`.`grades` `g` join `pasta_db`.`subjects` `s` on ((`g`.`subjectId` = `s`.`id`)));

create definer = root@`%` view pasta_db.v_subject_grade_summary as
select `s`.`userId`                                                                                   AS `userId`,
       `s`.`id`                                                                                       AS `subjectId`,
       coalesce(`s`.`finalGrade`, (sum((`g`.`value` * `g`.`weight`)) / nullif(sum(`g`.`weight`), 0))) AS `grade`
from (`pasta_db`.`subjects` `s` left join `pasta_db`.`grades` `g`
      on (((`g`.`subjectId` = `s`.`id`) and (`g`.`userId` = `s`.`userId`))))
group by `s`.`userId`, `s`.`id`, `s`.`finalGrade`;

create definer = root@`%` view pasta_db.v_schedule_display as
select `se`.`id`          AS `scheduleEntryId`,
       ...
       `ct`.`label`       AS `classType`
from ((`pasta_db`.`schedule_entries` `se` left join `pasta_db`.`subjects` `s`
       on ((`s`.`id` = `se`.`subjectId`))) left join `pasta_db`.`class_types` `ct`
      on ((`ct`.`id` = `se`.`classTypeId`)))
order by `se`.`startsAt`;

create definer = root@`%` view pasta_db.v_subject_details as
...
from ((((`pasta_db`.`subjects` `s` join `pasta_db`.`semesters` `sem`
         on ((`sem`.`id` = `s`.`semesterId`))) left join `pasta_db`.`subject_teachers` `st`
        on ((`st`.`subjectId` = `s`.`id`))) left join `pasta_db`.`subject_class_types` `stct`
       on ((`stct`.`subjectId` = `s`.`id`))) left join `pasta_db`.`class_types` `ct`
      on ((`ct`.`id` = `stct`.`classTypeId`)))
where (`sem`.`isCurrent` = 1)
group by `s`.`id`;

create definer = root@`%` view pasta_db.v_semester_subject_summary as
...
from (((`pasta_db`.`semesters` `sem` left join `pasta_db`.`subjects` `s`
        on (((`s`.`semesterId` = `sem`.`id`) and (`s`.`userId` = `sem`.`userId`))))
    left join `pasta_db`.`v_subject_grade_summary` `g`
       on (((`g`.`userId` = `sem`.`userId`) and (`g`.`subjectId` = `s`.`id`))))
    left join `pasta_db`.`v_subject_attendance_summary` `a`
      on (((`a`.`userId` = `sem`.`userId`) and (`a`.`subjectId` = `s`.`id`))));