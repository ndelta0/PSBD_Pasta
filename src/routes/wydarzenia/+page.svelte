<script lang="ts">
	...
	const initialEvents: EventItem[] = [
		...
		},
		{
			...
			time: '23:59',
			subject: 'Bazy danych',
			description: 'Projekt końcowy - system zarządznia bazą danych',
			type: 'Projekt',
			tone: 'amber'
		},
		{
		...
		},
		{
			...
			date: '2026-05-29',
			time: '14:15',
			subject: 'Algrytmy i struktury danych',
	...
	const addEvent = () => {
		...
		if (!title || !eventForm.date || !eventForm.timeHour || !eventForm.timeMinute || !subject || !description) {
			return;
		}
	...
	<section class="panel add-panel" aria-labelledby="add-event-title">
		...
		<form class="add-form" onsubmit={(event) => {
			...
			<div class="form-grid">
				...
				<label class="field-group full">
					<span>Opis</span>
					<textarea bind:value={eventForm.description} placeholder="Krótki opis wydarzenia" required></textarea>
				</label>
			</div>
			<div class="form-actions">
				<button class="ghost-button" type="button" onclick={() => (addEventOpen = false)}>Anuluj</button>
				<button class="action-button" type="submit"><span aria-hidden="true">+</span>Dodaj wydarzenie</button>
			</div>
		</form>
	</section>
{/if}
...
<section class="event-card-grid" aria-label="Lista wydarzeń">
	{#each events as event (event.id)}
		<article class={`panel event-card ${event.tone}`}>
			<header>
				<span class="nav-icon" data-icon="calendar" aria-hidden="true"></span>
				<h2>{event.title}</h2>
				<strong>{event.type}</strong>
			</header>
			<p class="event-date">
				<span class="mini-icon calendar-small" aria-hidden="true"></span>{event.date}
				<span class="mini-icon clock" aria-hidden="true"></span>{event.time}
			</p>
			<h3>{event.subject}</h3>
			<p class="event-description"><span class="mini-icon file" aria-hidden="true"></span>{event.description}</p>
		</article>
	{/each}
</section>
